...
public class JoinForm extends JFrame implements ActionListener, KeyListener {
	...
	JLabel joinTitleLb = new JLabel("회원가입");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == joinBtn) {
			...
			if(idFlag && pwdFlag1 && pwdFlag2 && nameFlag && telFlag && addrFlag ) {
				...
				JOptionPane.showMessageDialog(this, "회원가입이 완료됐습니다.", "",
	...
	public void validateTf() {
		...
		if(userId.equals("")) {
			...
			idFlag = false;
			inputValiLb.setText("아이디를 입력해주세요.");
			return;
		} else if(userId.length() < 4) {
			idVali.setIcon(xImg);
			showDialog("아이디는 4글자 이상 입력해주세요.");
			idTf.requestFocus(true);
			idFlag = false;
			inputValiLb.setText("아이디는 4글자 이상 입력해주세요.");
			return;
		} else {
			...
			if(result > 0) {
				...
				idTf.requestFocus(true);
				idFlag = false;
				inputValiLb.setText("중복된 아이디입니다.");
				return;
			} else {
				inputValiLb.setText("");
			}
		...
		if(userPwd1.equals("")) {
			...
			inputValiLb.setText("비밀번호를 입력해주세요.");
			pwdTf1.requestFocus(true);
			pwdFlag1 = false;
			return;
		} else if(userPwd1.length() < 4) {
			pwdVali1.setIcon(xImg);
			showDialog("비밀번호는 4글자 이상 입력해주세요.");
			inputValiLb.setText("비밀번호는 4글자 이상 입력해주세요.");
			pwdTf1.requestFocus(true);
			pwdFlag1 = false;
			return;
		} else {
		...
		if(userPwd2.equals("")) {
			...
			inputValiLb.setText("비밀번호 확인을 입력해주세요.");
			pwdTf2.requestFocus(true);
			pwdFlag2 = false;
			return;
		} else if(!userPwd2.equals(userPwd1)) {
			pwdVali2.setIcon(xImg);
			showDialog("비밀번호를 동일하게 입력해주세요.");
			inputValiLb.setText("비밀번호를 동일하게 입력해주세요.");
			pwdTf2.requestFocus(true);
			pwdFlag2 = false;
			return;
		} else {
		...
		if(userName.equals("")) {
			...
			inputValiLb.setText("이름을 입력해주세요.");
			nameTf.requestFocus(true);
			nameFlag = false;
			return;
		} else if(userName.length() < 2) {
			nameVali.setIcon(xImg);
			showDialog("이름을 정확히 입력해주세요.");
			inputValiLb.setText("이름을 정확히 입력해주세요.");
			nameTf.requestFocus(true);
			nameFlag = false;
			return;
		} else {
		...
		if(userTel.equals("")) {
			...
			showDialog("연락처를 입력해주세요.");
			inputValiLb.setText("연락처를 입력해주세요.");
			telTf.requestFocus(true);
			telFlag = false;
			return;
		} else if(userTel.length() < 11) {
			telVali.setIcon(xImg);
			showDialog("연락처를 정확히 입력해주세요.");
			inputValiLb.setText("연락처를 정확히 입력해주세요.");
			telTf.requestFocus(true);
			telFlag = false;
			return;
		} else {
			...
			if(!flag) {
				telVali.setIcon(xImg);
				showDialog("연락처는 숫자만 입력해주세요.");
				inputValiLb.setText("연락처는 숫자만 입력해주세요.");
				telTf.requestFocus(true);
				telFlag = false;
				return;
			} else {
		...
		if(userAddr.equals("")) {
			...
			inputValiLb.setText("주소를 입력해주세요.");
			addrTf.requestFocus(true);
			addrFlag = false;
			return;
		} else if(userAddr.length() < 5) {
			addrVali.setIcon(xImg);
			showDialog("주소를 정확히 입력해주세요.");
			inputValiLb.setText("주소를 정확히 입력해주세요.");
			addrTf.requestFocus(true);
			addrFlag = false;
			return;
		} else {
	...
	public void showDialog(String message) {
		JOptionPane.showMessageDialog(this, message, "오류",
	...
	public void keyReleased(KeyEvent e) {
		...
		if(e.getSource() == pwdTf2) {
			...
			if(userPwd2.length() < 4) {
				...
				pwdFlag1 = false;
				pwdFlag2 = false;
				inputValiLb.setText("비밀번호 확인을 정확히 입력해주세요.");
